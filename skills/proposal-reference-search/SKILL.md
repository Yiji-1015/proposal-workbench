...
사용자에게 후보와 세션 URL을 보고한다. 사용자가 "골랐어" 또는 "선택했어"라고 하면 세션의 `selected_slide_ids`를 읽어 선택 결과를 보고하고 종료한다. 이 Skill은 `proposal-slide-planner`를 호출하지 않는다. 완료된 세션은 사용자가 나중에 명시적으로 전달할 때만 장표 기획의 선택적 구조 레퍼런스로 사용한다.

피커의 `레퍼런스 자유 검색` 입력창에서 질의를 바꾸면 같은 세션의 후보가 새로 검색되고 기존 선택은 초기화된다. 렌더링 PNG가 없거나 파일이 사라진 후보는 깨진 이미지 대신 `html_ref` 구조 미리보기를 사용한다.

카드는 명시적인 선택 상태와 선택 개수를 표시한다. 구조 레퍼런스 선택에는 원본 덱 제한이 없으므로 여러 PPTX의 장표를 함께 골라도 된다. 피커는 선택과 확정에만 집중하며 PPTX 다운로드를 노출하지 않는다. 사용자가 원본 장표 파일을 별도로 요청할 때만 `selected_slide_ids`를 원본 덱별로 묶어 추출한다. `PNG 다운로드`는 이미지 보관용 보조 기능이다.
...
색인이 비어 있거나 부족하더라도 주변 폴더에서 발견한 PPTX/POTX를 자동 인제스트하지 않는다. 후보 파일명과 개수를 먼저 제시하고 사용자의 명시적 승인을 받은 뒤 `proposal-ppt-ingest`를 호출한다. 사용자가 검색을 요청한 사실만으로 회사자료나 첨부 참고자료의 인제스트에 동의한 것으로 해석하지 않는다.
