...
1. **요구사항 ID를 가장 먼저 확정한다.** 사용자가 대상 ID를 명시하지 않았다면 문서 유형, 전체/골격 범위, 평가 배점, 목표 분량을 제안하기 전에 “어떤 요구사항 번호를 작성할까요?”라고 묻는다. 선택한 ID 밖의 일반현황·회사소개·전체 제안서 범위를 임의로 확장하지 않는다.
2. **방향과 색상을 승인받는다.** 대상 ID를 확정한 다음 가로형(`landscape`)·세로형(`portrait`)과 주조색을 묻는다. 별도 색상 지시가 없으면 파란색 `#1769E0`과 남색 `#123B78`을 사용한다. 청록색을 임의의 주조색으로 선택하지 않는다.
...
2. 사용자에게 작성할 요구사항 ID를 가장 먼저 확인한다. 사용자가 ID를 지정했다면 그 범위를 승인된 것으로 본다.
3. 방향과 색상 팔레트를 승인받는다.
...
7. 선택한 자산 구조를 네이티브 PowerPoint 도형으로 재구성하는 청사진과 매핑을 만든다. 최종 `render_mode`는 `native_powerpoint_shapes`여야 한다.
...
- 최종 장표의 카탈로그 기반 도식은 `native_powerpoint_shapes`로 생성됐다.
- 카탈로그 기반 도식에 삽입 이미지(`p:pic`)와 미디어 파일이 없다. 사용자가 별도로 제공한 사진·로고를 명시적으로 요청한 경우만 예외다.
