...
8. 선택 에셋 고유의 구조가 보이는 첫 와이어프레임을 채팅에 인라인으로 표시한다. 사용자 명시 응답(예: “승인”, “진행”)을 받을 때까지 멈추고, 그 전에는 청사진·세션 상태를 `draft`/`pending`으로 유지한다. 최소 5개의 독립된 내용 상자와 `density: high`를 충족하되 같은 카드 형태를 반복하지 않는다.
9. 사용자 승인 상태가 확인된 프로젝트만 최종 렌더링한다. 승인 전에는 PPTX 생성 명령을 실행하지 않는다.
