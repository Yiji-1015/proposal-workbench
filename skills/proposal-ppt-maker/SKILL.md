...
6. **2차: 블록마다 자산을 고른다.** 카탈로그를 `semantic` → `structural` → `decorative` 순으로 검토해 참고할 자산을 찾는다. 방향 차이만으로 자산을 거절하지 않는다. 블록별로 고른 자산과 그 이유, 또는 맞는 자산이 없다는 판단 근거를 `usage_note`에 남긴다.
