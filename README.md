...
> **Agent 중심 독립 Skill 체계 + Human-in-the-Loop 보조 UI 기반의 제안 업무 자동화 플랫폼**

---

## 1. 아키텍처 개요

이 프로젝트는 단일 모놀리식 웹 서비스가 아닌, **Codex / Claude 같은 Agent가 제안 업무 단계별로 필요한 전문 Skill을 독립적으로 호출하고, 시각적 판단이 필요한 순간에만 Human-in-the-Loop (HitL) 브라우저 UI를 활용하는 분산 워크벤치**입니다.
...
```text
  ...
  └───────┬────────┘└─────┬──────┘└─────┬──────┘└─────┬──────┘└─────┬──────┘└──────┬───────┘
          ...
          │               ▼             │             ▼             ▼              │
          │     ┌──────────────────┐    │       ┌────────────────────────┐         │
          │     │ company_profile/ │    │       │ HitL Bridge & UI       │         │
          │     │ (제안사 프로필)  │    │       │ • Reference Picker     │         │
          │     └──────────────────┘    │       │ • Blueprint Reviewer   │         │
          │                             │       └────────────────────────┘         │
          ▼                             ▼                                          ▼
  ┌────────────────┐           ┌────────────────┐                         ┌────────────────┐
...
│  ├─ proposal-reference-search/     ──▶ ES KNN 벡터 검색 + HitL Reference Picker 안내
...
│  ├─ hitl-bridge/                   ──▶ 세션 JSON 교환 초경량 로컬 브릿지 서버 (포트 5174)
...
│
├─ ui/                               ──▶ Lovable deal-mechanic (HitL 보조 UI)
│  ├─ src/routes/search.tsx          ──▶ HitL Reference Picker (카드 선택 & 확정)
│  └─ src/routes/planning.tsx        ──▶ HitL Blueprint Reviewer (청사진 검토 & 승인)
│
...
### 2) 제안서 PPT 인제스트 (COM 렌더링 + 구조 추출)
```powershell
py -3.13 tools/ppt-ingest/ingest_pipeline.py --pptx "경로/제안서.pptx"
```

### 3) HitL 브릿지 서버 실행 (포트 5174)
```powershell
node tools/hitl-bridge/bridge_server.mjs
```

### 4) Lovable HitL UI 실행 (포트 5173)
```powershell
cd ui
bun dev
```
...
2. **레퍼런스 탐색**: `$proposal-reference-search` 실행 → 브라우저 피커 링크 열기 → 후보 슬라이드 선택 후 "골랐어".
3. **장표 기획**: `$proposal-slide-planner` 실행 → 브라우저 청사진 리뷰어 링크 열기 → 문구 검토 후 "승인했어".
