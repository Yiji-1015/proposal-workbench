#!/usr/bin/env python3
"""Read native PPTX/POTX package structure for ingest.

OOXML is read directly from the package so POTX files work even when python-pptx
rejects their template content type. The reader returns scope-aware shape
dictionaries (master, layout, slide) and the text/identity subset the ingest
pipeline indexes.
"""
