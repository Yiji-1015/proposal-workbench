...
5. 서버가 로컬 후보를 만들고 익명화 결과를 보여준다.
6. 사용자가 최종 승인하면 영구 카탈로그로 승격한다.

새 API는 다음 두 개로 제한한다.

- `POST /api/assets/discover`: `source_key`, `slide_no`를 받아 후보 목록을 반환한다.
- `POST /api/assets/promote`: `candidate_id`, `module_id`, `module_type`, `asset_kind`, `usage_mode`를 받아 검증 후 카탈로그로 승격한다.
...
대상을 하나로 확정할 수 없으면 Skill은 장표 하나를 물어본다. 후보 탐색 후에는 자동 승격하지 않고 블록 번호, 저장 범위(`블록 외형`, `내부 도식`, `둘 다`), 추론한 `module_type`, 지원 노드 수, 제거될 원본 문구를 보여준 뒤 승인받는다. Skill은 ingest UI와 같은 `tools/asset-curator` 코어를 호출한다.
...
11. 기존 빈 카탈로그 폴백과 기존 14개 renderer 회귀 테스트를 통과시킨다.
12. 실제 PowerPoint에서 wide·portrait·landscape 샘플을 열고 전체 슬라이드 PNG 내보내기를 통과시킨다.
